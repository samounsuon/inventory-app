    ...
    function table() {
        ...
        // Retrieve the last productId from localStorage for the current folder index, default to 1 if not found
        let productId = localStorage.getItem(`${getIndex}_lastProductId`) || 1;
        ...
        submitBtn.addEventListener("click", () => {
            ...
            tdI.addEventListener("click", () => {
                ...
                // After deletion, update productId
                productId = updateProductIdAfterDeletion(getIndex); 
            });

            // Save the updated productId to localStorage for the current folder index
            localStorage.setItem(`${getIndex}_lastProductId`, productId);
        });
        ...
        // Update productId after deleting a product
        function updateProductIdAfterDeletion(index) {
            let products = JSON.parse(localStorage.getItem(index)) || [];
            // If products exist, set the next productId to the highest ID + 1, else reset to 1
            if (products.length > 0) {
                let maxId = Math.max(...products.map(product => parseInt(product.tdId)));
                return maxId + 1;
            }
            return 1;
        }
